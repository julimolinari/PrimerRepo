...
//nombre de la pantalla contratada, precio, dias y precio total
int pub_findDaysOver(Publicidad* pPublicidad,Publicidad* pPantalla, int len, int idE)
{
    int i;
    int ret=-1;
    for(i=0;i<len;i++)
    {
        ...
        {
            Pantalla idPantalla;
            pan_getById(pPantalla,10,pPublicidad[i].idPantalla,idPantalla);
            printf("\nID: %d\nNombre: %s\nDireccion: %s\nPrecio: %.2f\nTipo: %d\n-------",
                   pPantalla[i].idPantalla,pPantalla[i].nombre,pPantalla[i].direccion,pPantalla[i].precio,pPantalla[i].tipo);
